...
describe("Alert.vue", () => {
  ...
  it("should allow cutsom content via slots", () => {
    const wrapper = mount(Alert, {
      props: {
        title: "test title",
      },
  ...
  it("should support centering text", () => {
    ...
    // class
    const rootNode = wrapper.find(".melon-alert");
  ...
  it("should not render close icon when closable is false", () => {
    ...
    expect(wrapper.find(".melon-alert__close").exists()).toBe(false);
  });
  ...
  it("should toggle visibility when open and close methods are called", async () => {
    ...
    expect(wrapper.find(".melon-alert").attributes().style).toBe("display: none;");
    await wrapper.vm.open();
    expect(wrapper.find(".melon-alert").attributes().style).toBe("");
  });
});
