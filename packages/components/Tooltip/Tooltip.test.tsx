...
describe("Tooltip/index.ts", () => {
  ...
  it("should be exported with withInstall", () => {
    expect(MelonTooltip.install).toBeDefined();
  });

  // 测试 Tooltip 组件是否被正确导出
  it("shiould be exported Tooltip components", () => {
    expect(MelonTooltip).toBe(Tooltip);
  });

  // 可选：测试 withInstall 是否增强了 Tooltip 组件的功能
  test("should enhance Tooltip component", () => {
    const enhanceTooltip = withInstall(Tooltip);
    expect(enhanceTooltip).toBe(MelonTooltip);
    // 可以添加更多测试，确保withInstall 增强了组件的特定功能
  });
  //   可选：如果withInstall 函数有特定的行为或属性，确保它们被正确使用
  test("should apply specific enhancements", () => {
    const enhancedTooltip = withInstall(Tooltip);
    // 例：如果你的withInstall 增加了一个特定的方法或属性
...
describe("Tooltip.vue", () => {
  ...
  test("basic tooltip", async () => {
    ...
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeFalsy();

    // 弹出层是否出现
    triggerArea.trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeTruthy();
    expect(wrapper.get(".melon-tooltip__popper").text()).toBe("hello tooltip");
  ...
  test("tooltip with hover trigger", async () => {
    ...
    // 测试悬停显示
    wrapper.find(".melon-tooltip__trigger").trigger("mouseenter");
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeTruthy();

    // 测试悬外隐藏
    wrapper.find(".melon-tooltip").trigger("mouseleave");
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeFalsy();
  });

  // 右键菜单触发的测试
  test("tooltip with contextmenu trigher", async () => {
    // 省略其他设置
    const wrapper = mount(Tooltip, {
      props: { trigger: "contextmenu", content: "test" },
    });
    // 测试右键菜单显示
    wrapper.find(".melon-tooltip__trigger").trigger("contextmenu");
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeTruthy();
    // 测试右键菜单隐藏（可以模拟点击外部区域）
  });
  ...
  test("tooltip with manual trigger", async () => {
    ...
    // 测试手动触发显示和隐藏
    wrapper.vm.show(); // 假设show方法可以通过某周方式访问
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeTruthy();
    wrapper.vm.hide();
    await vi.runAllTimers();
  ...
  test("disabled tooltip", async () => {
    ...
    // 测试禁用状态下点击不会触发显示
    wrapper.find(".melon-tooltip__trigger").trigger("click");
    await vi.runAllTimers();
    expect(wrapper.find(".melon-tooltip__popper").exists()).toBeFalsy();
  });
